...
def chef_create_component(component: Component) -> Component:
    try:
        ...
        raise SlugTaken(component.slug) from exc
    return component.model_copy(update={"id": str(result.inserted_id)})



def chef_list_components_by_ids(ids: Sequence[str]) -> dict[str, Component]:
    """Components by id for the chef, keyed by id, whatever their visibility.

    The prep sheet's read. Named `chef_*` because it deliberately sees
    what a customer read would refuse — an archived or withdrawn component
    ...
    return {
        str(document["_id"]): Component.model_validate(document)
...
def chef_update_component(component_id: str, fields: dict) -> bool:
    """Write the editor's fields onto one component. True when it landed.
...
def chef_set_allergens(component_id: str, allergens: dict) -> bool:
    """Write the allergen block onto one component. True when it landed.

    Its own function, deliberately, rather than a call to
    `chef_update_component` with one more key. 01-DOMAIN.md: allergen
...
def chef_set_sort_order(component_id: str, sort_order: int) -> bool:
    """Move one component in the chef's ordering."""
    object_id = to_object_id(component_id)
    if object_id is None:
        return False
    result = get_db()[COLLECTION].update_one(
